...
int main() {
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase) {
        ...
        // tu add kare ne agar 1 to last digit 9 hoy to y can be 8 less than x
        // aagad if 99 hoy to y can be 17 ocha than x, so 8, 17, 26, 35, 44, 53, 62, 71, 80
        // have normal case ma ek vadhare hoy to pan thay jaay
        // sum of digits of n and (n + 1) can never be same
        bool possible = false;
        if(y == x + 1) possible = true;
        else if(y < x) {
            ll diff = x - y;
            if(diff % 9 == 8)
                possible = true;
