...
int main() {
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase) {
        ...
        // so 1 in the main string can be replaced if there is a 0 on the previous index in the secondary string
        // now we can replace the 1's with 0's if it is even places away
        ll n;
        cin >> n;
        ...
        ll ones_a_odd = 0, ones_a_even = 0;
        ll zeros_b_odd = 0, zeros_b_even = 0;
        
        for(int i = 0; i < n; i++) {
            if((i+1) % 2 == 1) {  
                if(a[i] == '1') ones_a_odd++;
                if(b[i] == '0') zeros_b_odd++;
            } 
            else {  
                if(a[i] == '1') ones_a_even++;
                if(b[i] == '0') zeros_b_even++;
            }
        }
        
        if(ones_a_odd <= zeros_b_even && ones_a_even <= zeros_b_odd) {
            cout << "YES" << endl;
        } 
        else {
            cout << "NO" << endl;
