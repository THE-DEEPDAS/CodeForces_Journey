...
int main()
{
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase)
    {
        ...
        // number ma + 6 kari sake if num = 3 che andar, + 2 kari sake if num = 2 che andar
        // number of 2 time me 2 add kari saku and number of 3 times 6 add kari saku chu
        ll rem = num % 9;
        if (rem == 0)
        {
            cout << "YES\n";
        ...
        bool possible = false;
        for (int add2 = 0; add2 <= twos; add2++)
        {
            for (int add6 = 0; add6 <= threes; add6++)
            {
                if ((rem + add2 * 2 + add6 * 6) % 9 == 0)
                {
                    possible = true;
                    break;
                }
