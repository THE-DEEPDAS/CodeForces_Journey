...
pair<ll,ll> possible_divide(ll l, ll r, ll k){
    if(r - l + 1 < k) return {0 ,0};
    if(r - l + 1 == 1) return {1 ,1};
 
    ll mid = l + (r - l)/2, len = r - l + 1;
    if(len % 2){
        auto at = possible_divide(l, mid - 1, k);
        ll ans = mid + 2 * at.first + mid * at.second;
        return {ans, 2 * at.second + 1};
    }
    else{
        auto at = possible_divide(l, mid, k);
        ll ans = 2 * at.first + mid * at.second;
        return {ans, 2 * at.second};
    }
}
...
int main()
{
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase)
    {
        ...
        ll n, k;
        cin >> n >> k;
        auto ans = possible_divide(1, n, k);
        cout << ans.first << '\n';
    }
}
