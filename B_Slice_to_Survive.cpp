...
int main() {
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase) {
       ...
       ll n, m, a, b;
        ...
        ll min_from_a = min(a, n - a + 1);
        ll min_from_b = min(b, m - b + 1);
        
        // Calculate how many cuts needed for each dimension
        ll row_cuts = ceil(log2(n));
        ll col_cuts = ceil(log2(m));
        
        // Calculate answer based on total cuts needed
        cout << row_cuts + col_cuts - (row_cuts + col_cuts - ceil(log2(min_from_a)) - ceil(log2(min_from_b)) == 2) << '\n';
    }
}
