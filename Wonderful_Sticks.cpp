// si = <, to next stick baddhi pehla vadi karta nani hovi joiye
// si = >, to next stick baddhi pachi vadi karta moti hovi joiye
...
int main()
{
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase)
    {
        ...
        for (ll i = n - 2; i >= 0; i--)
        {
            if (s[i] == '>')
            {
                ans = to_string(bada) + ' ' + ans; 
                bada--;
            }
            else
            {
                ans = to_string(chota) + ' ' + ans; 
                chota++;
            }
        }
        ans = to_string(chota) + ' ' + ans; 
