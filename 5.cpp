...
int main() {
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase) {
        ...
        vector<ll> subArr, outArr;
        for (ll i = 0; i < n; i++) {
            if (i >= l-1 && i < r) {
                subArr.push_back(a[i]);
            } 
            else {
                outArr.push_back(a[i]);
            }
        }

        sort(subArr.begin(), subArr.end(), greater<ll>());
        sort(outArr.begin(), outArr.end());

        ll initial_sum = 0;
        for(auto &val : subArr) initial_sum += val;
        ll min_sum = initial_sum;

        ll max_replacements = min((ll)subArr.size(), (ll)outArr.size());

        vector<ll> prefix_sub(subArr.size() + 1, 0);
        vector<ll> prefix_out(outArr.size() + 1, 0);
        for(ll i = 0; i < subArr.size(); i++) {
            prefix_sub[i+1] = prefix_sub[i] + subArr[i];
        }
        for(ll i = 0; i < outArr.size(); i++) {
            prefix_out[i+1] = prefix_out[i] + outArr[i];
        }

        for(ll k = 0; k <= max_replacements; k++) {
            if(k == 0) {
                min_sum = min(min_sum, initial_sum);
                continue;
            }
            if(k > subArr.size() || k > outArr.size()) break;
            if(subArr[k-1] > outArr[k-1]) {
                ll current_sum = initial_sum - prefix_sub[k] + prefix_out[k];
