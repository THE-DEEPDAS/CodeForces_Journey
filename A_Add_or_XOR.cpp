...
int main()
{
    ...
    for (ll testcase = 0; testcase < testcases; ++testcase)
    {
        ...
        if (a > b)
        {
            // 5, 4 che maani lo to +1 kare to ek bit ma fer aavi jaay which is last second pan xor karvathi done
            // 6, 5 che to already 1 bit alag che so +1 kare to not possible on doing anything
            if (a == b + 1 && b % 2 == 0)
            {
                cout << y << "\n";
            }
        ...
        {
            // ahiya a < b che so me +1 karto rahu to answer is always possible
            // but we can save the cost and make the last bit as 1 if y < x
            // have aapde samajhye ke y operation karvathi thase shu
            // so xor karvathi jyare mare + karvanu che so last digit 0 hase ne hu XOR karu to e 1 thai jase, just like addition
            // but if e 1 hase to 0 thase which i want never as final number jo mare 0 bit hoy last to pehla mare ene 1 lagi lai jaavi pade
            // which will be extra cost of x and if last bit is 1 wanted then 2nd condition of minimum
            ll diff = b - a;
            if (diff % 2)
            {
                // now select karvu pade ke diff / 2 kayu karvanu ane diff / 2 + 1 kayu
                // so if last bit is 0 then i will do x operation one time extra else y operation
                // if(b % 2){
                //     cout <<  (diff / 2) * x + (diff / 2 + 1) * y << "\n";
                // }
                // else{
                //     cout << (diff / 2 + 1) * x + (diff / 2) * y << "\n";
                // }
            ...
            {
                cout << min(diff * x, (diff / 2) * x + (diff / 2) * y) << "\n";
            }
        }
    }
}
